# 07. log-제너레이터 (코드작성)
시청시간 관련 로그 발생 조건
-contents-start가 발생하면 contents-stop,access-out 로그 두개가 반드시 발생해야한다. 
-설정값의 활성도등급 상,중,하 비율에 따라 contents-start 로그 시간과 contents-stop,access-out의 로그 시간의 간격이 달라져야한다.(시청시간으로 집계시 사용을 위해서)
